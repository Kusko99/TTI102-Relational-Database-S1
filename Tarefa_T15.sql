...
create table fabricantes (idFabricante integer not null primary key, descricaoFabricante char(80) not null ) ;
...
create table vendedores (idVendedor integer not null primary key, nomeVendedor char(80) not null, cpfVendedor char(14) not null unique, quotaVendas decimal(8,2) not null);

insert into vendedores (idVendedor, nomeVendedor, cpfVendedor, quotaVendas) values 
...
create table clientes (idcliente integer not null primary key, nomeCliente char(80) not null, cpfCliente char(14) not null unique, foneCliente char(20) null);

insert into clientes (idCliente, nomeCliente, cpfCliente, foneCliente) values 
...
create table  pedidos (idPedido integer not null primary key, dataPedido date not null, valorPedido decimal (8,2) not null, idCliente integer not null, idVendedor integer not null,
foreign key(idCliente) references clientes(idCliente) , 
foreign key(idVendedor) references vendedores(idVendedor));


insert into pedidos (idPedido, dataPedido, valorPedido, idCliente, idVendedor) values 
(10, '2020-10-10', 23000.60, 9000, 1056),
  ...
  create table produtos (idProduto integer not null primary key, descProduto char(80) not null, idFabricante integer not null, 
  ...
  select * from produtos;
    ...
    create table itensPedidos (idPedido integer not null, idProduto integer not null, qtdePedida integer not null, 
    ...
    SELECT ROUND(AVG(quotaVendas),2) FROM Vendedores;
    
SELECT COUNT(idVendedor) FROM Vendedores;

SELECT nomeVendedor, quotaVendas FROM Vendedores
ORDER BY quotaVendas DESC
LIMIT 1;

SELECT nomeVendedor, quotaVendas FROM Vendedores
ORDER BY quotaVendas ASC
LIMIT 1;

SELECT SUM(quotaVendas) FROM Vendedores;

SELECT COUNT(idVendedor) FROM Vendedores
WHERE quotaVendas > 8000.00;

SELECT COUNT(idPedido) FROM Pedidos
WHERE idVendedor = 1056;

SELECT ROUND(SUM(valorPedido),2) FROM pedidos
WHERE idVendedor = 1002;

SELECT idPedido, dataPedido FROM pedidos
ORDER BY dataPedido ASC
LIMIT 1;

SELECT idPedido, valorPedido FROM pedidos
WHERE valorPedido > 12000.00;

SELECT idVendedor, ROUND(AVG(valorPedido),2) AS Media_Pedidos FROM pedidos
GROUP BY idVendedor
ORDER BY Media_Pedidos;

SELECT idVendedor, COUNT(idPedido) FROM pedidos
GROUP BY idVendedor;

SELECT idVendedor, MAX(valorPedido) AS Maior_pedido, MIN(valorPedido) as Menor_pedido FROM pedidos
GROUP BY idVendedor;

SELECT idVendedor, MAX(valorPedido) AS Maior_pedido, MIN(valorPedido) as Menor_pedido FROM pedidos
GROUP BY idVendedor
HAVING SUM(valorPedido) >= 10000;

SELECT idVendedor, COUNT(idPedido) AS Quantidade_Pedidos FROM pedidos
WHERE valorPedido>1000
GROUP BY idVendedor
ORDER BY idVendedor;

SELECT idVendedor, COUNT(idPedido) AS Quantidade_Pedidos, MAX(valorPedido) AS Maior_pedido, MIN(valorPedido) as Menor_pedido FROM pedidos
WHERE valorPedido>1000
GROUP BY idVendedor
ORDER BY idVendedor;

SELECT idVendedor, AVG(valorPedido) AS ValorMedio FROM pedidos
WHERE valorPedido > 20000
GROUP BY idVendedor;

SELECT idVendedor, quotaVendas
FROM Vendedores
WHERE quotaVendas < (SELECT AVG(quotaVendas) FROM Vendedores);