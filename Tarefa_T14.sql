...
create table patio (idPatio integer unique not null primary key, 
...
create table veiculo (idVeiculo int not null unique primary key, placa char(10) not null, modelo varchar(45) not null, cor varchar(45) not null, idcliente integer,
...
SELECT c.nomeCliente AS NomeCliente , v.Placa AS Placa
FROM cliente c JOIN veiculo v ON c.idCliente = v.idCliente;

SELECT c.nomeCliente AS NomeCliente , COALESCE(v.Placa,"Sem Veículo") AS Placa
FROM cliente c LEFT JOIN veiculo v ON c.idCliente = v.idCliente
ORDER BY c.nomeCliente;

SELECT COALESCE(c.nomeCliente,"Sem dono") AS NomeCliente , v.Placa AS Placa
FROM cliente c RIGHT JOIN veiculo v ON c.idCliente = v.idCliente
ORDER BY c.nomeCliente;

SELECT c.cpf AS cpf, c.nomeCliente AS NomeCliente
FROM cliente c JOIN veiculo v ON c.idCliente = v.idCliente
WHERE v.Placa = "ZU78JU49";

SELECT v.modelo AS Modelo, v.placa AS Placa, v.cor AS Cor
FROM veiculo v JOIN estaciona e ON v.idVeiculo = e.idVeiculo
WHERE e.idPatio = 1;
...
SELECT v.modelo AS Modelo, v.placa AS Placa, v.cor AS Cor
FROM veiculo v JOIN estaciona e ON v.idVeiculo = e.idVeiculo
WHERE e.idPatio IS NOT NULL;

SELECT v.modelo AS Modelo, v.placa AS Placa, v.cor AS Cor
FROM veiculo v JOIN estaciona e ON v.idVeiculo = e.idVeiculo
WHERE e.idPatio IS NOT NULL AND v.cor = 'Prata';


SELECT v.modelo AS Modelo, v.placa AS Placa, v.cor AS Cor
FROM veiculo v JOIN estaciona e ON v.idVeiculo = e.idVeiculo
WHERE e.idPatio IS NOT NULL AND v.cor != 'Vermelho';

SELECT v.modelo AS Modelo, v.placa AS Placa, e.dataEntrada AS DataEntrada
FROM veiculo v JOIN estaciona e ON v.idVeiculo = e.idVeiculo
WHERE e.idPatio IS NOT NULL AND v.cor != 'Vermelho';

SELECT v.modelo AS Modelo, v.placa AS Placa, e.dataEntrada AS DataEntrada
FROM veiculo v JOIN estaciona e ON v.idVeiculo = e.idVeiculo
WHERE e.idPatio IS NOT NULL AND e.dataEntrada BETWEEN '2021-01-01' AND '2021-12-31';
